...
import { 
  Plus, Search, Edit2, Database, 
  Cpu, Zap, ShieldCheck, Box,
  Info, Save, X, CheckCircle, 
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
        <div style={{ position: 'relative', width: '300px' }}>
          ...
          <input 
            ...
            style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', padding: '0.6rem 1rem 0.6rem 2.75rem', fontSize: '0.75rem', color: 'var(--white)', outline: 'none', borderRadius: '4px' }}
          />
        </div>
            ...
            {parts.length === 0 ? (
              <tr>
                <td colSpan={7} style={{ padding: '4rem', textAlign: 'center', opacity: 0.3, fontSize: '0.7rem', fontWeight: 800, letterSpacing: '0.1em' }}>
                  {isLoading ? 'INITIALIZING REGISTRY...' : 'NO COMPONENTS DEFINED'}
                </td>
              </tr>
            ) : (
              parts.map(part => (
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .icon-btn { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); color: rgba(255,255,255,0.3); padding: 0.5rem; border-radius: 2px; cursor: pointer; }
        .icon-btn:hover { color: var(--white); border-color: rgba(255,255,255,0.2); }
      `}} />
    </div>
  );
}
